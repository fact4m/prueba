<template>
    <div>
        <div class="page-header pr-0">
            ...
            <div v-if="admin" class="right-wrapper pull-right">
                    ...
                    <tr slot-scope="{ index, row }" :class="{'text-danger': (row.state_type_id === '11')}">
                        ...
                        <td class="text-center">
                            ...
                            <button type="button" class="btn waves-effect waves-light btn-xs btn-danger"
                                    @click.prevent="clickDeleteDocumentDemo(row.id)"
                                    v-if="row.btn_delete_demo">Eliminar</button>
                            <template v-if="admin">
                                <button type="button" class="btn waves-effect waves-light btn-xs btn-info"
                        ...
                        <template v-if="admin">
                            ...
                            <td class="text-right">
                                ...
                                <!--<button type="button" class="btn waves-effect waves-light btn-xs btn-info"-->
                                        <!--@click.prevent="clickUpdate(row.id)">Actualizar</button>-->
                            </td>
                        </template>
                    </tr>
                </data-table>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            clickUpdate(document_id) {
                this.$http.get(`/${this.resource}/update/${document_id}`)
                    .then(response => {
                        if (response.data.success) {
            ...
            clickDeleteDocumentDemo(recordId) {
                this.$http.delete(`/${this.resource}/${recordId}`)
                    .then(response => {
                        if (response.data.success) {
                            this.$message.success('Documento eliminado correctamente.')
                            this.$eventHub.$emit('reloadData')
                        } else {
                            this.$message.error('Error al intentar eliminar documento.')
                        }
                    })
                    .catch(error => {
                        this.$message.error(error.response.data.message)
